...
int main(){
    ...
    while(true){
        ...
        if(!strncmp(Command, "TURN ", 4)){
            ...
            if(clock() - timeOfTurn <= 500) iterativeDeepening(bestVal);
            currentTime = clock();
            printMove();
...
inline static void iterativeDeepening(int preBest){
    for(Depth = 13; ; Depth++){
        if(clock() - timeOfTurn > 1000) return;
        int bestVal = minMax(true, 0, -Inf, Inf);
        if(clock() - timeOfTurn >= 1600) return;
        if(bestVal >= preBest){
            deepeningCommand = moveCommand;
            preBest = bestVal;
        } 
...
inline static int evaluateScore(bool isMaxNode){/*should be much more complex, needed to be changed*/
        ...
        if(c == 0 || c == SIZE - 1){
            if(isMaxNode){
                if(boundaryJump(r, c, myColor)) myScore += HALF(CHESSVALUE);
                    else if(canBeKing(r, c, myColor)) myScore += HALF(KINGVALUE) + QUARTER(KINGVALUE);
                else myScore -= QUARTER(CHESSVALUE);
            }
        }
    ...
    for(int i = PIECE + 1; i <= PIECE << 1; i++){
        ...
        if(c == 0 || c == SIZE - 1) opponentScore -= QUARTER(CHESSVALUE);
        if(opponentColor == WHITE && !pBoard->isKing) {
            if(r >= 7) opponentScore += r << 3;
        } 
