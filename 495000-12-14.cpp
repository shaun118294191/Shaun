...
const int MAXCNT = 4950000;
const int MINCNT = 560000;
//CLOCKS_PER_SEC
/********************/
#define SIZE 8 
#define PIECE 12
...
inline static void depthManager(){ /**O2 13 not O2 12***/
    ...
    #ifdef FAST
        Depth = 10;
        if(curStep >= 52) Depth = 60 - curStep + 2;
    #endif
    #ifndef FAST
        Depth = 11;
    #endif
...
inline static int evaluateScore(bool isMaxNode){/*should be much more complex, needed to be changed*/
    ...
    for(int i = 1; i <= PIECE; i++){
        ...
        if(chessThreatened(r, c, myColor)){
            ...
            if(pBoard->isKing) myScore -= HALF(KINGVALUE);
            else myScore -= HALF(CHESSVALUE); //上一层为minNode 敌方已做完决策
        } 
        #endif
    ...
    for(int i = PIECE + 1; i <= PIECE << 1; i++){
        ...
        if(chessThreatened(r, c, opponentColor)){
            ...
            if(pBoard->isKing) opponentScore -= HALF(KINGVALUE);
            else opponentScore -= HALF(CHESSVALUE); //上一层为minNode 敌方已做完决策
        }       
        #endif
