...
int main(){
    ...
    while(true){
        ...
        if(!strncmp(Command, "TURN ", 4)){
            ...
            } else Depth = (60 - curStep) >= 11 ? 11 : curStep + 1;
            /******/
            getRestChess();
            timeOfTurn = clock();
...
inline static double evaluateScore(void){/*should be much more complex, needed to be changed*/
    ...
    for(int i = 1; i <= PIECE; i++){
        ...
        if(chessThreatened(r, c, myColor)) myScore -= 1.82;
    ...
    for(int i = PIECE + 1; i <= PIECE << 1; i++){
        ...
        if(chessThreatened(r, c, opponentColor)) opponentScore -= 1.32;    
...
double minMax(bool isMaxNode, int depth, double alpha, double beta){
    ...
    if(clock() - timeOfTurn >= 1700){
        double cutOffScore = evaluateScore();
        return cutOffScore > 0.0 ? cutOffScore / 2.2 : cutOffScore * 1.2;  
    }
