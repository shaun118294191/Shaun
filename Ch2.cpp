...
chess Chess[PIECE << 1], canMoveChess[MAXMCNT];
    ...
    for(int i = startI; i < endI; i++){
        ...
        for(int j = 0; j < 4; j++){
            ...
            if(Board[nextR][nextC].color == EMPTY){
                if(!visChess)//canMoveChess line 64
                    canMoveChess[cntChess++] = Chess[i], visChess = true;
                cntMoves++; 
                if(cntMoves >= limit) return cntChess - 1;
            }
            int jumpR = r + 2 * dr[i], jumpC = c + 2 * dc[i];
            if(jumpR >= SIZE || jumpR < 0 || jumpC >= SIZE || jumpC < 0) 
                continue;
...
int minMax(bool isMaxNode, byte r, byte c, byte depth, int alpha, int beta){
    ...
    if(isMaxNode){
        bestVal = Inf;  
        cntCh = getPossilbeMoves(0, 14, 0, myColor);
        //id 0~11 mychess  id 12~23 opponent chess
        for(int i = 0; i < PIECE; i++){
