/**
 * 3rd Dec 
 * choice of jump and store path should be done
 * **/
...
double makeJump(bool isMaxNode, byte color, byte depth, byte cJump, byte r, byte c, double alpha, double beta);
/*******function**********/
int main(){
...
double makeJump(bool isMaxNode, byte color, byte depth, byte cJump, byte r, byte c, double alpha, double beta){
    ...
    if(cJump == longestJump){
        ...
        return minMax(isMaxNode ^ 1, depth + 1, alpha, beta); 
