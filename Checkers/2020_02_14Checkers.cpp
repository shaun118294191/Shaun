...
time_t timeOfTurn;
time_t currentTime;
const time_t timeLimit = 1810;
time_t firstMinMaxTime;
int Depth;
...
const int scoreWhite[SIZE][SIZE] = {
    0, 200, 0, 200, 0, 200, 0, 190,
    190, 0, 180, 0, 180, 0, 180, 0,
    0, 170, 0, 170, 0, 170, 0, 170,
    170, 0, 160, 0, 160, 0, 160, 0,
    0, 160, 0, 160, 0, 160, 0, 160,
    130, 0, 150, 0, 150, 0, 150, 0,
    0, 140, 0, 140, 0, 140, 0, 120,
    110, 0, 130, 0, 130, 0, 130, 0
};
const int scoreBLACK[SIZE][SIZE] = {
    0, 110, 0, 130, 0, 130, 0, 130, 
    120, 0, 140, 0, 140, 0, 140, 0,
    0, 150, 0, 150, 0, 150, 0, 130,
    160, 0, 160, 0, 160, 0, 160, 0,
    0, 160, 0, 160, 0, 160, 0, 170,
    170, 0, 170, 0, 170, 0, 170, 0,
    0, 180, 0, 180, 0, 180, 0, 190,
...
inline static void depthManager(){ /**O2 11 not O2 9***/
    curStep++;
    getRestChess();
    #ifdef FAST
        Depth = 12;
        if(curStep >= 51) Depth = 10;
        //if(firstMinMaxTime >= timeLimit) Depth = 9;
    #endif
    #ifndef FAST
        Depth = 10;
    #endif
...
inline static void iterativeDeepening(int preBest){
    for(Depth = (Depth == 10) ? 12 : 14; ; Depth +=2){ //每次迭代加深2层深度
        #ifdef Debug
            printf("iterativeDeepened Depth = %d cnt = %d time %.3f\n", Depth, 0, (clock() - timeOfTurn) / (double)CLOCKS_PER_SEC );
        #endif
        int bestVal = minMax(true, 0, -Inf, Inf);//搜索
        #ifdef Debug
            printf("iterativeDeepened Depth = %d cnt = %d time %.3f\n", Depth, 0, (clock() - timeOfTurn) / (double)CLOCKS_PER_SEC );
        #endif
        time_t deepeningTime = clock() - timeOfTurn;
        if(deepeningTime >= 205) {//测试发现 多搜2层会多耗费6-7倍的时间
            if(deepeningTime < timeLimit) deepeningCommand = moveCommand, preBest = bestVal;
            //没有截断 则选择更深层数得到的最优解
            else if(bestVal >= preBest - 20){//被截断 与之前最优解比较 选择局部最优解 
                deepeningCommand = moveCommand;
                preBest = bestVal;
            } 
            return;
...
inline static int evaluateScore(bool isMaxNode){/*should be much more complex, needed to be changed*/
    ...
    const int (*pMyScore)[8] = myColor == WHITE ? scoreWhite : scoreBLACK;
    const int (*pOpponentScore)[8] = myColor == WHITE ? scoreBLACK : scoreWhite; 
    for(int i = 1; i <= PIECE; i++){
        chess * pChess = &Chess[i];
        if(pChess->color == EMPTY) continue;
        int r = pChess->r, c = pChess->c;
        board * pBoard = &Board[r][c];
        myScore += pBoard->isKing ? pMyScore[r][c] + KINGVALUE : pMyScore[r][c];
    } 
    for(int i = PIECE + 1; i <= PIECE << 1; i++){
        chess * pChess = &Chess[i];
        if(pChess->color == EMPTY) continue;
        int r = pChess->r, c = pChess->c;
        board * pBoard = &Board[r][c];
        opponentScore += pBoard->isKing ? pOpponentScore[r][c] + KINGVALUE : pOpponentScore[r][c];
    }
...
int minMax(bool isMaxNode, int depth, int alpha, int beta){
    ...
    if(!isMaxNode){
        ...
        memcpy(copyMoveChess, moveChess, sizeof(copyMoveChess));
         /**jumpChess moveChess copy**/
        if(copyCntJump){
            for(int i = 0; i < copyCntJump; i++){ //执行跳吃走法
                if(copyJumpChess[i].lgstJump < copyLongestJump) continue;
            ...
            for(int i = 0; i < cntChess; i++){ //执行移动走法
