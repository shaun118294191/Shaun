...
inline static double evaluateScore/*Score evaluation*/(bool isMaxNode);
...
inline static void depthManager(){ /**O2 13 not O2 12***/
    ...
    #ifdef FAST
        Depth = 12;
        if(curStep <= 50 && !(curStep & 1)) Depth = 13;
        if(myResChess < 5) Depth = 13;
        if(4 <= myResChess && myResChess <= 6 && 4 <= opponentResChess && opponentResChess <= 6) Depth = 12;
        if(curStep >= 50) Depth = 10;
    #endif
    #ifndef FAST
        Depth = 11;
        if(myResChess >= 8) Depth = 11;
        if(curStep >= 20 && curStep <= 50) Depth = 12;
        if(myResChess <= 6) Depth = 12;
    #endif
}
inline static bool isNotInBound(int r, int c){
...
inline static double evaluateScore(bool isMaxNode){/*should be much more complex, needed to be changed*/
    ...
    for(int i = 1; i <= PIECE; i++){
        ...
        if(pBoard->isKing) myScore += 9.0;
            else myScore += 3.0;
        if(chessThreatened(r, c, myColor)){
            if(pBoard->isKing) myScore -= (isMaxNode ? 3.0 : 6.0);
            else myScore -= (isMaxNode ? 1.40 : 1.32); //上一层为minNode 敌方已做完决策
        } 
        if(c == 0 || c == SIZE - 1){
            if(isMaxNode){
                if(boundaryJump(r, c, myColor)) myScore += 2.0;
                    else if(canBeKing(r, c, myColor)) myScore += 6.0;
                else myScore -= 1.32;
            }
        }
    ...
    for(int i = PIECE + 1; i <= PIECE << 1; i++){
        ...
        chess * pChess = &Chess[i];
        if(pChess->color == EMPTY) continue;
        if(pBoard->isKing) opponentScore += 9.0;
            else opponentScore += 3.0;
        if(chessThreatened(r, c, opponentColor)){
            if(pBoard->isKing) opponentScore -= (isMaxNode ? 6.0 : 3.0); //上一层为minNode 敌方已做完决策
            else opponentScore -= (isMaxNode ? 1.32 : 1.40); //上一层为minNode 敌方已做完决策
        }       
        if(c == 0 || c == SIZE - 1) opponentScore -= 1.12;
...
double minMax(bool isMaxNode, int depth, double alpha, double beta){
    ...
    if(clock() - timeOfTurn >= 1800){
        double cutOffScore = evaluateScore(isMaxNode);
        if(isMaxNode) cutOffScore = cutOffScore > 0.0 ? cutOffScore * 2.0 : cutOffScore / 2.0;
            else cutOffScore = cutOffScore > 0.0 ? cutOffScore / 2.0 : cutOffScore * 2.0;
        return cutOffScore;  
    }
    ...
    if(isMaxNode){
        ...
        cntChess = getPossilbeMoves(myResChess > 7 ? 8 : myResChess, 0, myColor);
    ...
    if(!isMaxNode){
        value = Inf, bestVal = Inf, longestJump = 0, cntJump = 0/*related to jump chess*/;
        cntChess = getPossilbeMoves(opponentResChess > 7 ? 8 : opponentResChess, 0, opponentColor);
        copyLongestJump = longestJump, copyCntJump = cntJump;
        memcpy(copyJumpChess, jumpChess, sizeof(copyJumpChess));
        memcpy(copyMoveChess, moveChess, sizeof(copyMoveChess));
         /**jumpChess moveChess copy**/
